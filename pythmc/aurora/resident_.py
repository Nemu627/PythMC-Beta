...
class Resident:
    ...
    def __init__(self, name: str, *, data: Tuple[dict, dict] = None):
        ...
        if not hasattr(self, "town"):
            self.town = next((town_.Town(town_name, data=data) for town_name in data[0] if name in data[0][town_name]["desc"][4]), None)
    ...
    def all(cls, *, data: Tuple[dict, dict] = None) -> Set[Resident]:
        if data is None:
            data = get.get_data()
        return {resident for town in town_.Town.all(data=data) for resident in town.residents}
