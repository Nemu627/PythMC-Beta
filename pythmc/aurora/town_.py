...
from . import get, nation_, resident_


class Town:
    name: str
    nation: nation_.Nation
    colour: str
    mayor: resident_.Resident
    residents: list[resident_.Resident]
    flags: Dict[str, bool]
    area: int
    ...
    def __init__(self, name: str, *, data: Tuple[dict, dict] = None):
        ...
        if not hasattr(self, "nation"):
            ...
            if nation == "":
                ...
                self.nation = nation_.Nation(nation, data=data)
        self.colour = data[0][name]["fillcolor"]
        self.mayor = resident_.Resident._with_town(data[0][name]["desc"][2], data, self)
        self.residents = [resident_.Resident._with_town(person, data, self) for person in data[0][name]["desc"][4].split(", ")]
        self.flags = {
